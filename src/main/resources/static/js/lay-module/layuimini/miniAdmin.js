...
layui.define(["jquery", "miniMenu", "element", "miniTab", "miniTheme"], function (exports) {
    ...
    var miniAdmin = {
         ...
         * @param options.header token
         * @param options.username 登陆的用户名
         */
        render: function (options) {
            ...
            $.ajax({
                headers: header,
                async: false,
                url: GWDTZ_BASE_URL + '/home/getModuleList',
                type: "get",
                data: {
                ...
                success: function (data) {
                    if (data.data.flag) {
                        layer.msg("没有获得菜单权限，请联系上一级！", {icon: 0});
                        setTimeout(_=>{
                        ...
                        var deptlevelcode = data.data.deptlevelcode;
                        var homeInfo = {};
                        if(deptlevelcode.length == 2){
                            homeInfo['title'] = '用户维护';
                            homeInfo['href'] = 'view/sys/userManager.html';
                        }else {
                            if(deptlevelcode.substring(0,4) == '0001'){
                                homeInfo['title'] = '发布督办';
                                homeInfo['href'] = 'view/sys/ContentManager-fbdb.html?id=21';
                            }else {
                                homeInfo['title'] = '技术员';
                                homeInfo['href'] = 'view/sys/schedule/ScheduleManager.html?persontype=1';
                            }
                        }

                        var logoInfo = {};
                        logoInfo['title'] = '车间内业管理系统';
                        logoInfo['image'] = 'images/railway.png';
                        logoInfo['href'] = '';
                        var object = {};
                        object['homeInfo'] = homeInfo;
                        object['logoInfo'] = logoInfo;
                        object['menuInfo'] = data.data.list;
                        var data = object;
        ...
        renderLogo: function (data) {
            var html = '<a href="' + data.href + '"><img src="' + data.image + '" alt="logo"><p style="    display: inline-block;\n' +
                '    margin-top: 5px;\n' +
                '    color: #ffffff;\n' +
                '    font-size: 16px;\n' +
                '    margin-left: -10px;">' + data.title + '</p></ast>';
