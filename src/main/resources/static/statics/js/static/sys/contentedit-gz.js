...
function Bind(contentId) {
	ajaxGet("/content/selectContentById", function(ajaxRet) {
		...
		$("input[name='implementtime']").val(ajaxRet.contentDO.updatetime);
		$("input[name='contents']").val(ajaxRet.contentDO.contents);
		$("input[name='title']").val(ajaxRet.contentDO.title);
		$("input[name='serial']").val(ajaxRet.contentDO.serial);
	...
	if(contentId == -1) {
		$("#infoForm").validate({
			...
			submitHandler: function(form) {
				$(form).ajaxSubmit({
					headers: header,
					url: GWDTZ_BASE_URL + '/content/contentAdd-gz',
					type: "post",
					dataType: "json",
					data:{
						username:username
		...
		$("#infoForm").validate({
			...
			submitHandler: function(form) {
				$(form).ajaxSubmit({
					headers: header,
					url: GWDTZ_BASE_URL + '/content/contentEdit-gz',
					type: "put",
					dataType: "json",
					data:{
						username:username
