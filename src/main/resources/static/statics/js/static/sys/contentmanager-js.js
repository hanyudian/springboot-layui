	...
	$.ajax({
		...
		success: function(data) {
			...
			$.each(list, function(i, row) {
				...
				infoHtml += '<td><a class="btn-visibleplant"  onclick="toPerform1(\'' + row.id + '\')">可见车间</a></td>';
				infoHtml += '<td>' + row.updatetime + '</td>';
				infoHtml += '<td>' + row.publishuser + '</td>';
...
function toPerform(contentId) {
	layer.open({
		...
		fixed: true,
		skin: 'layui-layer-rim',
		content:'ContentEdit.html?contentId='+contentId+'&moduleId='+moduleId,
		success:function (layero,index) {
			var body=layer.getChildFrame("body",index);
