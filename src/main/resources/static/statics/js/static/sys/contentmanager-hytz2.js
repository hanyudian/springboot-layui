...
function contentListInit() {
	...
	$.ajax({
		...
		type: 'get',
		url: GWDTZ_BASE_URL + '/content/getContentList3',
		data: {
			pageNum: pager.index,
			pageSize: pager.size,
			username:username,
		...
		success: function(data) {
			...
			$.each(list, function(i, row) {
				...
				infoHtml += '<td><a class="btn-completion"  onclick="toPerform1(\'' + row.id + '\')">落实情况</a></td>';
				if(row.status == 1 &&  offsetTime1 > 0) infoHtml += '<td>' + '<span class="icon11">今日到期未开会</span>' + '</td>';
				else if(row.status == 1) infoHtml += '<td>' + '<span class="icon1">未开会</span>' + '</td>';
				else if(row.status == 2) infoHtml += '<td>' + '<span class="icon2">' + "逾期" + offsetDays + "天" +"未开会"+'</span>' + '</td>';
...
function toPerform1(contentid) {
	layer.open({
		...
		content:'complete.html?contentid='+contentid,
