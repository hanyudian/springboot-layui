	...
	this.render = function(){
		...
		if(self._pageCount > 1){
			if(self.index != 1){
				...
				str += '<a href="javascript://' + (self.index-1) + '"><span>上一页</span></a>';
			}else{
				str += '<span>|&lt;</span>';
				str += '<span>上一页</span>';
			}
		...
		if(self._pageCount > 1){
			if(self.index != self._pageCount){
				str += '<a href="javascript://' + (self.index+1) + '"><span>下一页</span></a>';
		...
		if(self._pageCount>1)str += ' &nbsp;&nbsp;&nbsp;跳转<input class="gotoPage" style="width:60px;height:18px;line-height:18px;" type="text" onchange="javascript://"/>页 ';
		...
		if(gotopageInput!=null && gotopageInput[0]!=null)gotopageInput[0].onchange = function(){
			...
			if(index != undefined && index != ''){
				...
				if(index!=null && !isNaN(index)){
					if(index<1)index=1;
					if(index>self._pageCount)index=1;
					self.index = index;
					self._onclick(index);
				}else{
					alert("请输入正确的页码");
